...
- **Metadata store.** The ref → clonepack mapping and build status. SQLite is the only supported database; temporary file and object-storage ref stores remain for legacy rollback. Writes are ordered so a newer sync never loses to an older one.
