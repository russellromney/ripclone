...
- **`matrix.sh`** — sweep cores, RTT, and bandwidth across clone modes (`full`/`hybrid`/`fast`).
...
- **`profile_one.sh`** — quick single-cell profile of `full` vs `fast` through a shaped proxy.
...
- `RIPCLONE_BLOB_PACK_THREADS` — threads for local pack building in `full` mode.
