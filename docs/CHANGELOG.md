...
- **URL refresh is operation-local and shared across artifacts**: a clone retains the newest complete signed-URL response. Queued artifacts select from that response when they start, and concurrent rejections share one exact-commit refresh instead of regenerating and discarding the full URL set once per artifact.
